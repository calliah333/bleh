...
export function collage({ host, sidebar } = {}) {
	...
	function make_collage(bypass = false) {
		if (
			...
			parseInt(width.current.value) < min ||
			parseInt(width.current.value) > max ||
			parseInt(height.current.value) < min ||
			parseInt(height.current.value) > max
		) {
			notify({
				id: 'collage_failed',
		...
		try {
			...
			let collage_dom = html.node`
				...
				settings.collage_title
					? html.node`
                    <div class="header">
                        <div class="type" data-type=${type.current.value}>
                            <div class="bleh-icon" />
                            ...
                            <strong>${
						timeframe_text(timeframe.current.value)
					}</strong>
                            <strong>${
